...
export default {
  ...
  getParts(params = {}) { return api.get('/parts', { params }) },
  getPart(id) { return api.get(`/parts/${id}`) },
  createPart(data) { return api.post('/parts', data) },
  updatePart(id, data) { return api.put(`/parts/${id}`, data) },
  deletePart(id) { return api.delete(`/parts/${id}`) },
  exportParts() { return api.get('/parts/export', { responseType: 'blob' }) },
  importPartsPreview(file) { const f = new FormData(); f.append('file', file); return api.post('/parts/import/preview', f, { headers: { 'Content-Type': 'multipart/form-data' } }) },
  importPartsConfirm(file, rows) { const f = new FormData(); f.append('file', file); f.append('rows', JSON.stringify(rows)); return api.post('/parts/import/confirm', f, { headers: { 'Content-Type': 'multipart/form-data' } }) },
  getPartsImportHistory(params = {}) { return api.get('/parts/import/history', { params }) },
  downloadPartsImportFile(id) { return api.get(`/parts/import/history/${id}/download`, { responseType: 'blob' }) },
  deletePartsImportHistory(id) { return api.delete(`/parts/import/history/${id}`) },
  getPartsSummary(params = {}) { return api.get('/parts-summary', { params }) },
  exportPartsSummary(params = {}) { return api.get('/parts-summary/export', { params, responseType: 'blob' }) },
  getPartsUsageByCode(code, params = {}) { return api.get(`/parts-summary/by-code/${encodeURIComponent(code)}`, { params }) },
