...
export default {
  ...
  setup() {
    ...
    const form = ref({ customer_id: '', name: '', address: '' }); const error = ref(''); const loading = ref(false)
    ...
    const handleSubmit = async () => {
      ...
      try {
        const payload = { customer_id: form.value.customer_id, name: form.value.name, address: form.value.address }
        if (isEdit.value) {
