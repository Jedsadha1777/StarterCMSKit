...
class PreviewShell extends StatefulWidget {
  ...
  /// Optional back callback — shown as a back button in the AppBar leading.
...
class _PreviewShellState extends State<PreviewShell> {
  ...
  // Edit mode strips the inner page margin so form content fills the paper —
  // easier to fill in when zoomed to paper-edge. Review mode shows the document
  // with its intended margin (looks like a printed page).
  EdgeInsets get _effectivePagePadding =>
      _reviewMode ? widget.pagePadding : EdgeInsets.zero;

  // Outer shell margin around paper. Edit mode zeroes it out so the paper
  // sits flush against viewport edges (no gray top/sides/bottom).
  ...
  void _onMockupPressed(String label) {
          ...
          leading: AnimatedSwitcher(
            ...
            child: _reviewMode
                ...
                : TextButton.icon(
                    ...
                    onPressed: widget.onBack ?? () => _onMockupPressed('Back'),
