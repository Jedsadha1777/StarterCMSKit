...
class ReportScreen2State extends State<ReportScreen2> {
  ...
  // When user picks a part from FormSearch — fill code + price + recalc total.
  void _onPartPicked(Map<String, dynamic>? v, {
    ...
    setState(() {
      ...
      final p = v['unit_price'];
      priceCtrl.text = p is num ? p.toStringAsFixed(2) : '';
      _recalcPartTotal(qtyCtrl, priceCtrl, totalCtrl);
      _renumberParts();
    });
  }

  void _recalcPartTotal(TextEditingController qty, TextEditingController price, TextEditingController total) {
    final q = int.tryParse(qty.text.trim()) ?? 0;
    final p = double.tryParse(price.text.trim()) ?? 0;
    final t = q * p;
    total.text = t > 0 ? t.toStringAsFixed(2) : '';
    _recalcPartSubTotal();
  }
  ...
  TextEditingController _partTotalCtrlFor(int i) => switch (i) {
    1 => _partTotal1Controller, 2 => _partTotal2Controller, 3 => _partTotal3Controller,
    4 => _partTotal4Controller, 5 => _partTotal5Controller, 6 => _partTotal6Controller,
    7 => _partTotal7Controller, 8 => _partTotal8Controller, 9 => _partTotal9Controller,
    10 => _partTotal10Controller, 11 => _partTotal11Controller, 12 => _partTotal12Controller,
    13 => _partTotal13Controller, 14 => _partTotal14Controller, 15 => _partTotal15Controller,
    _ => throw RangeError('part row $i out of range'),
  };
  ...
  TextEditingController _unitPriceCtrlFor(int i) => switch (i) {
    1 => _unitPrice1Controller, 2 => _unitPrice2Controller, 3 => _unitPrice3Controller,
    4 => _unitPrice4Controller, 5 => _unitPrice5Controller, 6 => _unitPrice6Controller,
    7 => _unitPrice7Controller, 8 => _unitPrice8Controller, 9 => _unitPrice9Controller,
    10 => _unitPrice10Controller, 11 => _unitPrice11Controller, 12 => _unitPrice12Controller,
    13 => _unitPrice13Controller, 14 => _unitPrice14Controller, 15 => _unitPrice15Controller,
    _ => throw RangeError('part row $i out of range'),
  };
  ...
  // Clear row i and shift rows i+1..15 up. Recalc # + sub_total + grand_total.
  void _clearPartRow(int i) {
    ...
    setState(() {
      for (int k = i; k < 15; k++) {
        ...
        _unitPriceCtrlFor(k).text = _unitPriceCtrlFor(k + 1).text;
        _partQtyCtrlFor(k).text = _partQtyCtrlFor(k + 1).text;
        _partTotalCtrlFor(k).text = _partTotalCtrlFor(k + 1).text;
      }
      // Last row becomes blank.
      _setPartNameRaw(15, null);
      _partCodeCtrlFor(15).text = '';
      _unitPriceCtrlFor(15).text = '';
      _partQtyCtrlFor(15).text = '';
      _partTotalCtrlFor(15).text = '';
      _renumberParts();
      _recalcPartSubTotal();   // chains to _recalcPartGrandTotal
    });
  }

  // Part fee total — charge_rate × charge_hrs (both decimal-supporting).
  void _recalcPartFeeTotal() {
    final rate = double.tryParse(_chargeRateController.text.trim()) ?? 0;
    final hrs = double.tryParse(_chargeHrsController.text.trim()) ?? 0;
    final t = rate * hrs;
    _partFeeTotalController.text = t > 0 ? t.toStringAsFixed(2) : '';
    _recalcPartGrandTotal();
  }

  // Sub-total — sums all row totals (recalc every time any row total changes).
  void _recalcPartSubTotal() {
    double sum = 0;
    for (int i = 1; i <= 15; i++) {
      final v = double.tryParse(_partTotalCtrlFor(i).text.trim()) ?? 0;
      sum += v;
    }
    _partSubTotalController.text = sum > 0 ? sum.toStringAsFixed(2) : '';
    _recalcPartGrandTotal();
  }

  // Grand total — recalc dependents (sub_total + fee_total) then sum.
  void _recalcPartGrandTotal() {
    final sub = double.tryParse(_partSubTotalController.text.trim()) ?? 0;
    final fee = double.tryParse(_partFeeTotalController.text.trim()) ?? 0;
    final t = sub + fee;
    _partGrandTotalController.text = t > 0 ? t.toStringAsFixed(2) : '';
  }
  ...
  List<String> _missingRequired() {
    ...
    final hasAnyPart = List.generate(15, (idx) => _partNameFor(idx + 1))
      ...
      child: Stack(
        ...
        children: [
          ...
          Positioned(left: cs[3], top: rs[32], width: cs[21] - cs[3], height: rs[33] - rs[32], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: FormSearch(name: 'part_name_1', source: 'parts', displayFields: 'parts_code,parts_name,unit_price', fields: 'part_name_1,part_code1,unit_price1', enabled: _isPartRowEnabled(1), onCleared: () => _clearPartRow(1), snapMode: _snapMode, showValidation: _showValidation, value: _partName1, onSelected: (v) => _onPartPicked(v, setName: (n) { _partName1 = n; }, codeCtrl: _partCode1Controller, priceCtrl: _unitPrice1Controller, qtyCtrl: _partQty1Controller, totalCtrl: _partTotal1Controller)))),
          ...
          Positioned(left: cs[26], top: rs[32], width: cs[30] - cs[26], height: rs[33] - rs[32], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: ((_partNameFor(1) ?? "").isNotEmpty) ? TextField(controller: _partQty1Controller, keyboardType: TextInputType.number, textAlign: TextAlign.right, onChanged: (_) => _recalcPartTotal(_partQty1Controller, _unitPrice1Controller, _partTotal1Controller), style: const TextStyle(fontFamily: 'Calibri', fontSize: 14.6), decoration: _inputDecoration) : const SizedBox.shrink())),
          Positioned(left: cs[30], top: rs[32], width: cs[36] - cs[30], height: rs[33] - rs[32], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_unitPrice1Controller, textAlign: TextAlign.right))),
          Positioned(left: cs[36], top: rs[32], width: cs[45] - cs[36], height: rs[33] - rs[32], child: Container(
              ...
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_partTotal1Controller, fontSize: 14.6, textAlign: TextAlign.right))),
          ...
          Positioned(left: cs[3], top: rs[33], width: cs[21] - cs[3], height: rs[34] - rs[33], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: FormSearch(name: 'part_name_2', source: 'parts', displayFields: 'parts_code,parts_name,unit_price', fields: 'part_name_2,part_code2,unit_price2', enabled: _isPartRowEnabled(2), onCleared: () => _clearPartRow(2), snapMode: _snapMode, showValidation: _showValidation, value: _partName2, onSelected: (v) => _onPartPicked(v, setName: (n) { _partName2 = n; }, codeCtrl: _partCode2Controller, priceCtrl: _unitPrice2Controller, qtyCtrl: _partQty2Controller, totalCtrl: _partTotal2Controller)))),
          Positioned(left: cs[21], top: rs[33], width: cs[26] - cs[21], height: rs[34] - rs[33], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.center, child: _readOnlyTextField(_partCode2Controller, fontSize: 14.6))),
          Positioned(left: cs[26], top: rs[33], width: cs[30] - cs[26], height: rs[34] - rs[33], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: ((_partNameFor(2) ?? "").isNotEmpty) ? TextField(controller: _partQty2Controller, keyboardType: TextInputType.number, textAlign: TextAlign.right, onChanged: (_) => _recalcPartTotal(_partQty2Controller, _unitPrice2Controller, _partTotal2Controller), style: const TextStyle(fontFamily: 'Calibri', fontSize: 16), decoration: _inputDecoration) : const SizedBox.shrink())),
          Positioned(left: cs[30], top: rs[33], width: cs[36] - cs[30], height: rs[34] - rs[33], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_unitPrice2Controller, textAlign: TextAlign.right))),
          Positioned(left: cs[36], top: rs[33], width: cs[45] - cs[36], height: rs[34] - rs[33], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 2), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_partTotal2Controller, fontSize: 14.6, textAlign: TextAlign.right))),
          ...
          Positioned(left: cs[3], top: rs[34], width: cs[21] - cs[3], height: rs[35] - rs[34], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: FormSearch(name: 'part_name_3', source: 'parts', displayFields: 'parts_code,parts_name,unit_price', fields: 'part_name_3,part_code3,unit_price3', enabled: _isPartRowEnabled(3), onCleared: () => _clearPartRow(3), snapMode: _snapMode, showValidation: _showValidation, value: _partName3, onSelected: (v) => _onPartPicked(v, setName: (n) { _partName3 = n; }, codeCtrl: _partCode3Controller, priceCtrl: _unitPrice3Controller, qtyCtrl: _partQty3Controller, totalCtrl: _partTotal3Controller)))),
          Positioned(left: cs[21], top: rs[34], width: cs[26] - cs[21], height: rs[35] - rs[34], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.center, child: _readOnlyTextField(_partCode3Controller, fontSize: 14.6))),
          Positioned(left: cs[26], top: rs[34], width: cs[30] - cs[26], height: rs[35] - rs[34], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: ((_partNameFor(3) ?? "").isNotEmpty) ? TextField(controller: _partQty3Controller, keyboardType: TextInputType.number, textAlign: TextAlign.right, onChanged: (_) => _recalcPartTotal(_partQty3Controller, _unitPrice3Controller, _partTotal3Controller), style: const TextStyle(fontFamily: 'Calibri', fontSize: 14.6), decoration: _inputDecoration) : const SizedBox.shrink())),
          Positioned(left: cs[30], top: rs[34], width: cs[36] - cs[30], height: rs[35] - rs[34], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_unitPrice3Controller, textAlign: TextAlign.right))),
          Positioned(left: cs[36], top: rs[34], width: cs[45] - cs[36], height: rs[35] - rs[34], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 2), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_partTotal3Controller, fontSize: 14.6, textAlign: TextAlign.right))),
          ...
          Positioned(left: cs[3], top: rs[35], width: cs[21] - cs[3], height: rs[36] - rs[35], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: FormSearch(name: 'part_name_4', source: 'parts', displayFields: 'parts_code,parts_name,unit_price', fields: 'part_name_4,part_code4,unit_price4', enabled: _isPartRowEnabled(4), onCleared: () => _clearPartRow(4), snapMode: _snapMode, showValidation: _showValidation, value: _partName4, onSelected: (v) => _onPartPicked(v, setName: (n) { _partName4 = n; }, codeCtrl: _partCode4Controller, priceCtrl: _unitPrice4Controller, qtyCtrl: _partQty4Controller, totalCtrl: _partTotal4Controller)))),
          Positioned(left: cs[21], top: rs[35], width: cs[26] - cs[21], height: rs[36] - rs[35], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.center, child: _readOnlyTextField(_partCode4Controller, fontSize: 14.6))),
          Positioned(left: cs[26], top: rs[35], width: cs[30] - cs[26], height: rs[36] - rs[35], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: ((_partNameFor(4) ?? "").isNotEmpty) ? TextField(controller: _partQty4Controller, keyboardType: TextInputType.number, textAlign: TextAlign.right, onChanged: (_) => _recalcPartTotal(_partQty4Controller, _unitPrice4Controller, _partTotal4Controller), style: const TextStyle(fontFamily: 'Calibri', fontSize: 14.6), decoration: _inputDecoration) : const SizedBox.shrink())),
          Positioned(left: cs[30], top: rs[35], width: cs[36] - cs[30], height: rs[36] - rs[35], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_unitPrice4Controller, textAlign: TextAlign.right))),
          Positioned(left: cs[36], top: rs[35], width: cs[45] - cs[36], height: rs[36] - rs[35], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 2), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_partTotal4Controller, fontSize: 14.6, textAlign: TextAlign.right))),
          ...
          Positioned(left: cs[3], top: rs[36], width: cs[21] - cs[3], height: rs[37] - rs[36], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: FormSearch(name: 'part_name_5', source: 'parts', displayFields: 'parts_code,parts_name,unit_price', fields: 'part_name_5,part_code5,unit_price5', enabled: _isPartRowEnabled(5), onCleared: () => _clearPartRow(5), snapMode: _snapMode, showValidation: _showValidation, value: _partName5, onSelected: (v) => _onPartPicked(v, setName: (n) { _partName5 = n; }, codeCtrl: _partCode5Controller, priceCtrl: _unitPrice5Controller, qtyCtrl: _partQty5Controller, totalCtrl: _partTotal5Controller)))),
          Positioned(left: cs[21], top: rs[36], width: cs[26] - cs[21], height: rs[37] - rs[36], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.center, child: _readOnlyTextField(_partCode5Controller, fontSize: 14.6))),
          Positioned(left: cs[26], top: rs[36], width: cs[30] - cs[26], height: rs[37] - rs[36], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: ((_partNameFor(5) ?? "").isNotEmpty) ? TextField(controller: _partQty5Controller, keyboardType: TextInputType.number, textAlign: TextAlign.right, onChanged: (_) => _recalcPartTotal(_partQty5Controller, _unitPrice5Controller, _partTotal5Controller), style: const TextStyle(fontFamily: 'Calibri', fontSize: 14.6), decoration: _inputDecoration) : const SizedBox.shrink())),
          Positioned(left: cs[30], top: rs[36], width: cs[36] - cs[30], height: rs[37] - rs[36], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_unitPrice5Controller, textAlign: TextAlign.right))),
          Positioned(left: cs[36], top: rs[36], width: cs[45] - cs[36], height: rs[37] - rs[36], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 2), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_partTotal5Controller, fontSize: 14.6, textAlign: TextAlign.right))),
          ...
          Positioned(left: cs[3], top: rs[37], width: cs[21] - cs[3], height: rs[38] - rs[37], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: FormSearch(name: 'part_name_6', source: 'parts', displayFields: 'parts_code,parts_name,unit_price', fields: 'part_name_6,part_code6,unit_price6', enabled: _isPartRowEnabled(6), onCleared: () => _clearPartRow(6), snapMode: _snapMode, showValidation: _showValidation, value: _partName6, onSelected: (v) => _onPartPicked(v, setName: (n) { _partName6 = n; }, codeCtrl: _partCode6Controller, priceCtrl: _unitPrice6Controller, qtyCtrl: _partQty6Controller, totalCtrl: _partTotal6Controller)))),
          Positioned(left: cs[21], top: rs[37], width: cs[26] - cs[21], height: rs[38] - rs[37], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.center, child: _readOnlyTextField(_partCode6Controller, fontSize: 14.6))),
          Positioned(left: cs[26], top: rs[37], width: cs[30] - cs[26], height: rs[38] - rs[37], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: ((_partNameFor(6) ?? "").isNotEmpty) ? TextField(controller: _partQty6Controller, keyboardType: TextInputType.number, textAlign: TextAlign.right, onChanged: (_) => _recalcPartTotal(_partQty6Controller, _unitPrice6Controller, _partTotal6Controller), style: const TextStyle(fontFamily: 'Calibri', fontSize: 14.6), decoration: _inputDecoration) : const SizedBox.shrink())),
          Positioned(left: cs[30], top: rs[37], width: cs[36] - cs[30], height: rs[38] - rs[37], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_unitPrice6Controller, textAlign: TextAlign.right))),
          Positioned(left: cs[36], top: rs[37], width: cs[45] - cs[36], height: rs[38] - rs[37], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 2), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_partTotal6Controller, fontSize: 14.6, textAlign: TextAlign.right))),
          ...
          Positioned(left: cs[3], top: rs[38], width: cs[21] - cs[3], height: rs[39] - rs[38], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: FormSearch(name: 'part_name_7', source: 'parts', displayFields: 'parts_code,parts_name,unit_price', fields: 'part_name_7,part_code7,unit_price7', enabled: _isPartRowEnabled(7), onCleared: () => _clearPartRow(7), snapMode: _snapMode, showValidation: _showValidation, value: _partName7, onSelected: (v) => _onPartPicked(v, setName: (n) { _partName7 = n; }, codeCtrl: _partCode7Controller, priceCtrl: _unitPrice7Controller, qtyCtrl: _partQty7Controller, totalCtrl: _partTotal7Controller)))),
          Positioned(left: cs[21], top: rs[38], width: cs[26] - cs[21], height: rs[39] - rs[38], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.center, child: _readOnlyTextField(_partCode7Controller, fontSize: 14.6))),
          Positioned(left: cs[26], top: rs[38], width: cs[30] - cs[26], height: rs[39] - rs[38], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: ((_partNameFor(7) ?? "").isNotEmpty) ? TextField(controller: _partQty7Controller, keyboardType: TextInputType.number, textAlign: TextAlign.right, onChanged: (_) => _recalcPartTotal(_partQty7Controller, _unitPrice7Controller, _partTotal7Controller), style: const TextStyle(fontFamily: 'Calibri', fontSize: 14.6), decoration: _inputDecoration) : const SizedBox.shrink())),
          Positioned(left: cs[30], top: rs[38], width: cs[36] - cs[30], height: rs[39] - rs[38], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_unitPrice7Controller, textAlign: TextAlign.right))),
          Positioned(left: cs[36], top: rs[38], width: cs[45] - cs[36], height: rs[39] - rs[38], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 2), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_partTotal7Controller, fontSize: 14.6, textAlign: TextAlign.right))),
          ...
          Positioned(left: cs[3], top: rs[39], width: cs[21] - cs[3], height: rs[40] - rs[39], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: FormSearch(name: 'part_name_8', source: 'parts', displayFields: 'parts_code,parts_name,unit_price', fields: 'part_name_8,part_code8,unit_price8', enabled: _isPartRowEnabled(8), onCleared: () => _clearPartRow(8), snapMode: _snapMode, showValidation: _showValidation, value: _partName8, onSelected: (v) => _onPartPicked(v, setName: (n) { _partName8 = n; }, codeCtrl: _partCode8Controller, priceCtrl: _unitPrice8Controller, qtyCtrl: _partQty8Controller, totalCtrl: _partTotal8Controller)))),
          Positioned(left: cs[21], top: rs[39], width: cs[26] - cs[21], height: rs[40] - rs[39], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.center, child: _readOnlyTextField(_partCode8Controller, fontSize: 14.6))),
          Positioned(left: cs[26], top: rs[39], width: cs[30] - cs[26], height: rs[40] - rs[39], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: ((_partNameFor(8) ?? "").isNotEmpty) ? TextField(controller: _partQty8Controller, keyboardType: TextInputType.number, textAlign: TextAlign.right, onChanged: (_) => _recalcPartTotal(_partQty8Controller, _unitPrice8Controller, _partTotal8Controller), style: const TextStyle(fontFamily: 'Calibri', fontSize: 14.6), decoration: _inputDecoration) : const SizedBox.shrink())),
          Positioned(left: cs[30], top: rs[39], width: cs[36] - cs[30], height: rs[40] - rs[39], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_unitPrice8Controller, textAlign: TextAlign.right))),
          Positioned(left: cs[36], top: rs[39], width: cs[45] - cs[36], height: rs[40] - rs[39], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 2), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_partTotal8Controller, fontSize: 14.6, textAlign: TextAlign.right))),
          ...
          Positioned(left: cs[3], top: rs[40], width: cs[21] - cs[3], height: rs[41] - rs[40], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: FormSearch(name: 'part_name_9', source: 'parts', displayFields: 'parts_code,parts_name,unit_price', fields: 'part_name_9,part_code9,unit_price9', enabled: _isPartRowEnabled(9), onCleared: () => _clearPartRow(9), snapMode: _snapMode, showValidation: _showValidation, value: _partName9, onSelected: (v) => _onPartPicked(v, setName: (n) { _partName9 = n; }, codeCtrl: _partCode9Controller, priceCtrl: _unitPrice9Controller, qtyCtrl: _partQty9Controller, totalCtrl: _partTotal9Controller)))),
          Positioned(left: cs[21], top: rs[40], width: cs[26] - cs[21], height: rs[41] - rs[40], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.center, child: _readOnlyTextField(_partCode9Controller, fontSize: 14.6))),
          Positioned(left: cs[26], top: rs[40], width: cs[30] - cs[26], height: rs[41] - rs[40], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: ((_partNameFor(9) ?? "").isNotEmpty) ? TextField(controller: _partQty9Controller, keyboardType: TextInputType.number, textAlign: TextAlign.right, onChanged: (_) => _recalcPartTotal(_partQty9Controller, _unitPrice9Controller, _partTotal9Controller), style: const TextStyle(fontFamily: 'Calibri', fontSize: 14.6), decoration: _inputDecoration) : const SizedBox.shrink())),
          Positioned(left: cs[30], top: rs[40], width: cs[36] - cs[30], height: rs[41] - rs[40], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_unitPrice9Controller, textAlign: TextAlign.right))),
          Positioned(left: cs[36], top: rs[40], width: cs[45] - cs[36], height: rs[41] - rs[40], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 2), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_partTotal9Controller, fontSize: 14.6, textAlign: TextAlign.right))),
          ...
          Positioned(left: cs[3], top: rs[41], width: cs[21] - cs[3], height: rs[42] - rs[41], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: FormSearch(name: 'part_name_10', source: 'parts', displayFields: 'parts_code,parts_name,unit_price', fields: 'part_name_10,part_code10,unit_price10', enabled: _isPartRowEnabled(10), onCleared: () => _clearPartRow(10), snapMode: _snapMode, showValidation: _showValidation, value: _partName10, onSelected: (v) => _onPartPicked(v, setName: (n) { _partName10 = n; }, codeCtrl: _partCode10Controller, priceCtrl: _unitPrice10Controller, qtyCtrl: _partQty10Controller, totalCtrl: _partTotal10Controller)))),
          Positioned(left: cs[21], top: rs[41], width: cs[26] - cs[21], height: rs[42] - rs[41], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.center, child: _readOnlyTextField(_partCode10Controller, fontSize: 14.6))),
          Positioned(left: cs[26], top: rs[41], width: cs[30] - cs[26], height: rs[42] - rs[41], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: ((_partNameFor(10) ?? "").isNotEmpty) ? TextField(controller: _partQty10Controller, keyboardType: TextInputType.number, textAlign: TextAlign.right, onChanged: (_) => _recalcPartTotal(_partQty10Controller, _unitPrice10Controller, _partTotal10Controller), style: const TextStyle(fontFamily: 'Calibri', fontSize: 14.6), decoration: _inputDecoration) : const SizedBox.shrink())),
          Positioned(left: cs[30], top: rs[41], width: cs[36] - cs[30], height: rs[42] - rs[41], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_unitPrice10Controller, textAlign: TextAlign.right))),
          Positioned(left: cs[36], top: rs[41], width: cs[45] - cs[36], height: rs[42] - rs[41], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 2), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_partTotal10Controller, fontSize: 14.6, textAlign: TextAlign.right))),
          ...
          Positioned(left: cs[3], top: rs[42], width: cs[21] - cs[3], height: rs[43] - rs[42], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: FormSearch(name: 'part_name_11', source: 'parts', displayFields: 'parts_code,parts_name,unit_price', fields: 'part_name_11,part_code11,unit_price11', enabled: _isPartRowEnabled(11), onCleared: () => _clearPartRow(11), snapMode: _snapMode, showValidation: _showValidation, value: _partName11, onSelected: (v) => _onPartPicked(v, setName: (n) { _partName11 = n; }, codeCtrl: _partCode11Controller, priceCtrl: _unitPrice11Controller, qtyCtrl: _partQty11Controller, totalCtrl: _partTotal11Controller)))),
          Positioned(left: cs[21], top: rs[42], width: cs[26] - cs[21], height: rs[43] - rs[42], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.center, child: _readOnlyTextField(_partCode11Controller, fontSize: 14.6))),
          Positioned(left: cs[26], top: rs[42], width: cs[30] - cs[26], height: rs[43] - rs[42], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: ((_partNameFor(11) ?? "").isNotEmpty) ? TextField(controller: _partQty11Controller, keyboardType: TextInputType.number, textAlign: TextAlign.right, onChanged: (_) => _recalcPartTotal(_partQty11Controller, _unitPrice11Controller, _partTotal11Controller), style: const TextStyle(fontFamily: 'Calibri', fontSize: 14.6), decoration: _inputDecoration) : const SizedBox.shrink())),
          Positioned(left: cs[30], top: rs[42], width: cs[36] - cs[30], height: rs[43] - rs[42], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_unitPrice11Controller, textAlign: TextAlign.right))),
          Positioned(left: cs[36], top: rs[42], width: cs[45] - cs[36], height: rs[43] - rs[42], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 2), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_partTotal11Controller, fontSize: 14.6, textAlign: TextAlign.right))),
          ...
          Positioned(left: cs[3], top: rs[43], width: cs[21] - cs[3], height: rs[44] - rs[43], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: FormSearch(name: 'part_name_12', source: 'parts', displayFields: 'parts_code,parts_name,unit_price', fields: 'part_name_12,part_code12,unit_price12', enabled: _isPartRowEnabled(12), onCleared: () => _clearPartRow(12), snapMode: _snapMode, showValidation: _showValidation, value: _partName12, onSelected: (v) => _onPartPicked(v, setName: (n) { _partName12 = n; }, codeCtrl: _partCode12Controller, priceCtrl: _unitPrice12Controller, qtyCtrl: _partQty12Controller, totalCtrl: _partTotal12Controller)))),
          Positioned(left: cs[21], top: rs[43], width: cs[26] - cs[21], height: rs[44] - rs[43], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.center, child: _readOnlyTextField(_partCode12Controller, fontSize: 14.6))),
          Positioned(left: cs[26], top: rs[43], width: cs[30] - cs[26], height: rs[44] - rs[43], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: ((_partNameFor(12) ?? "").isNotEmpty) ? TextField(controller: _partQty12Controller, keyboardType: TextInputType.number, textAlign: TextAlign.right, onChanged: (_) => _recalcPartTotal(_partQty12Controller, _unitPrice12Controller, _partTotal12Controller), style: const TextStyle(fontFamily: 'Calibri', fontSize: 14.6), decoration: _inputDecoration) : const SizedBox.shrink())),
          Positioned(left: cs[30], top: rs[43], width: cs[36] - cs[30], height: rs[44] - rs[43], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_unitPrice12Controller, textAlign: TextAlign.right))),
          Positioned(left: cs[36], top: rs[43], width: cs[45] - cs[36], height: rs[44] - rs[43], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 2), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_partTotal12Controller, fontSize: 14.6, textAlign: TextAlign.right))),
          ...
          Positioned(left: cs[3], top: rs[44], width: cs[21] - cs[3], height: rs[45] - rs[44], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: FormSearch(name: 'part_name_13', source: 'parts', displayFields: 'parts_code,parts_name,unit_price', fields: 'part_name_13,part_code13,unit_price13', enabled: _isPartRowEnabled(13), onCleared: () => _clearPartRow(13), snapMode: _snapMode, showValidation: _showValidation, value: _partName13, onSelected: (v) => _onPartPicked(v, setName: (n) { _partName13 = n; }, codeCtrl: _partCode13Controller, priceCtrl: _unitPrice13Controller, qtyCtrl: _partQty13Controller, totalCtrl: _partTotal13Controller)))),
          Positioned(left: cs[21], top: rs[44], width: cs[26] - cs[21], height: rs[45] - rs[44], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.center, child: _readOnlyTextField(_partCode13Controller, fontSize: 14.6))),
          Positioned(left: cs[26], top: rs[44], width: cs[30] - cs[26], height: rs[45] - rs[44], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: ((_partNameFor(13) ?? "").isNotEmpty) ? TextField(controller: _partQty13Controller, keyboardType: TextInputType.number, textAlign: TextAlign.right, onChanged: (_) => _recalcPartTotal(_partQty13Controller, _unitPrice13Controller, _partTotal13Controller), style: const TextStyle(fontFamily: 'Calibri', fontSize: 14.6), decoration: _inputDecoration) : const SizedBox.shrink())),
          Positioned(left: cs[30], top: rs[44], width: cs[36] - cs[30], height: rs[45] - rs[44], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_unitPrice13Controller, textAlign: TextAlign.right))),
          Positioned(left: cs[36], top: rs[44], width: cs[45] - cs[36], height: rs[45] - rs[44], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 2), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_partTotal13Controller, fontSize: 14.6, textAlign: TextAlign.right))),
          ...
          Positioned(left: cs[3], top: rs[45], width: cs[21] - cs[3], height: rs[46] - rs[45], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: FormSearch(name: 'part_name_14', source: 'parts', displayFields: 'parts_code,parts_name,unit_price', fields: 'part_name_14,part_code14,unit_price14', enabled: _isPartRowEnabled(14), onCleared: () => _clearPartRow(14), snapMode: _snapMode, showValidation: _showValidation, value: _partName14, onSelected: (v) => _onPartPicked(v, setName: (n) { _partName14 = n; }, codeCtrl: _partCode14Controller, priceCtrl: _unitPrice14Controller, qtyCtrl: _partQty14Controller, totalCtrl: _partTotal14Controller)))),
          Positioned(left: cs[21], top: rs[45], width: cs[26] - cs[21], height: rs[46] - rs[45], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.center, child: _readOnlyTextField(_partCode14Controller, fontSize: 14.6))),
          Positioned(left: cs[26], top: rs[45], width: cs[30] - cs[26], height: rs[46] - rs[45], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: ((_partNameFor(14) ?? "").isNotEmpty) ? TextField(controller: _partQty14Controller, keyboardType: TextInputType.number, textAlign: TextAlign.right, onChanged: (_) => _recalcPartTotal(_partQty14Controller, _unitPrice14Controller, _partTotal14Controller), style: const TextStyle(fontFamily: 'Calibri', fontSize: 14.6), decoration: _inputDecoration) : const SizedBox.shrink())),
          Positioned(left: cs[30], top: rs[45], width: cs[36] - cs[30], height: rs[46] - rs[45], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_unitPrice14Controller, textAlign: TextAlign.right))),
          Positioned(left: cs[36], top: rs[45], width: cs[45] - cs[36], height: rs[46] - rs[45], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 2), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_partTotal14Controller, fontSize: 14.6, textAlign: TextAlign.right))),
          ...
          Positioned(left: cs[3], top: rs[46], width: cs[21] - cs[3], height: rs[47] - rs[46], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 2))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: FormSearch(name: 'part_name_15', source: 'parts', displayFields: 'parts_code,parts_name,unit_price', fields: 'part_name_15,part_code15,unit_price15', enabled: _isPartRowEnabled(15), onCleared: () => _clearPartRow(15), snapMode: _snapMode, showValidation: _showValidation, value: _partName15, onSelected: (v) => _onPartPicked(v, setName: (n) { _partName15 = n; }, codeCtrl: _partCode15Controller, priceCtrl: _unitPrice15Controller, qtyCtrl: _partQty15Controller, totalCtrl: _partTotal15Controller)))),
          Positioned(left: cs[21], top: rs[46], width: cs[26] - cs[21], height: rs[47] - rs[46], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 2))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.center, child: _readOnlyTextField(_partCode15Controller, fontSize: 14.6))),
          Positioned(left: cs[26], top: rs[46], width: cs[30] - cs[26], height: rs[47] - rs[46], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 2))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: ((_partNameFor(15) ?? "").isNotEmpty) ? TextField(controller: _partQty15Controller, keyboardType: TextInputType.number, textAlign: TextAlign.right, onChanged: (_) => _recalcPartTotal(_partQty15Controller, _unitPrice15Controller, _partTotal15Controller), style: const TextStyle(fontFamily: 'Calibri', fontSize: 14.6), decoration: _inputDecoration) : const SizedBox.shrink())),
          Positioned(left: cs[30], top: rs[46], width: cs[36] - cs[30], height: rs[47] - rs[46], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 1), bottom: BorderSide(color: Color(0xFF000000), width: 2))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_unitPrice15Controller, textAlign: TextAlign.right))),
          Positioned(left: cs[36], top: rs[46], width: cs[45] - cs[36], height: rs[47] - rs[46], child: Container(
              ...
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_partTotal15Controller, fontSize: 14.6, textAlign: TextAlign.right))),
          ...
          Positioned(left: cs[36], top: rs[47], width: cs[45] - cs[36], height: rs[48] - rs[47], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 2), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_partSubTotalController, textAlign: TextAlign.right))),
          ...
          Positioned(left: cs[9], top: rs[48], width: cs[13] - cs[9], height: rs[49] - rs[48], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(bottom: BorderSide(color: Color.fromRGBO(0, 0, 0, 1), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: TextField(controller: _chargeRateController, textAlign: TextAlign.right, keyboardType: const TextInputType.numberWithOptions(decimal: true), onChanged: (_) => _recalcPartFeeTotal(), style: const TextStyle(fontFamily: 'Arial', fontSize: 17.3), decoration: _inputDecoration))),
          ...
          Positioned(left: cs[14], top: rs[48], width: cs[18] - cs[14], height: rs[49] - rs[48], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(bottom: BorderSide(color: Color.fromRGBO(0, 0, 0, 1), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: TextField(controller: _chargeHrsController, textAlign: TextAlign.right, keyboardType: const TextInputType.numberWithOptions(decimal: true), onChanged: (_) => _recalcPartFeeTotal(), style: const TextStyle(fontFamily: 'Arial', fontSize: 14.6), decoration: _inputDecoration))),
          ...
          Positioned(left: cs[36], top: rs[48], width: cs[45] - cs[36], height: rs[49] - rs[48], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 2), bottom: BorderSide(color: Color(0xFF000000), width: 1))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_partFeeTotalController, textAlign: TextAlign.right))),
          ...
          Positioned(left: cs[36], top: rs[49], width: cs[45] - cs[36], height: rs[50] - rs[49], child: Container(
              decoration: BoxDecoration(color: Colors.transparent, border: Border(right: BorderSide(color: Color(0xFF000000), width: 2), bottom: BorderSide(color: Color(0xFF000000), width: 2))),
              padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 4.0), alignment: Alignment.centerLeft, child: _readOnlyTextField(_partGrandTotalController, textAlign: TextAlign.right))),
